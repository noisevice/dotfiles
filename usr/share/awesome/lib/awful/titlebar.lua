...
local function new(c, args)
    ...
    local size = 24 --args.size or util.round(beautiful.get_font_height(args.font) * 1.5)
...
function titlebar.widget.button(c, name, selector, action)
    ...
    ret.update_onhover = update_onhover
    --ret.update_offhover = update_offhover
    ret.update_onpress = update_onpress

    ret:connect_signal("mouse::enter", update_onhover)
    ret:connect_signal("mouse::leave", update) --_offhover
    ret:connect_signal("button::press", update_onpress)
    ret:connect_signal("button::release", update_onhover)
